...
from model_gcn_cnn import DeepTTG 
import metrics
...
path = Path(f'result/DeepTTG_{datetime.now()}')
...
for phase_name in ['train', 'val', 'test2016', 'test2013']:
    dataset = data_loaders[phase_name].dataset
    if phase_name == 'train':
        reduced_indices = torch.arange(50)  # Use first 50 samples for training
    else:
        reduced_indices = torch.arange(10)  # Use first 10 samples for testing/validation
    data_loaders[phase_name] = DataLoader(
        dataset[reduced_indices],
        batch_size=24,
        pin_memory=True,
        shuffle=True
    )
